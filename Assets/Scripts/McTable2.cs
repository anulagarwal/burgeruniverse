...
public class McTable2 : MonoBehaviour
{
    ...
    {
        //animOfRotatingFoodIcon = GetComponentInChildren<Animation>();
        while (true)
        {
            if (animIsUp && !DoesNeedFood())
            {
                animIsUp = false;
                //animOfRotatingFoodIcon.Play("RotatingFoodIconDown");
            }
            else if (!animIsUp && DoesNeedFood())
            {
                //animOfRotatingFoodIcon.Play();
                animIsUp = true;
            }
    ...
    //public bool DoesHaveEmpty()
    //{
    //    return isEmpty.Contains(true);
    //}
